...
import static seedu.address.logic.commands.CommandTestUtil.SUBJECT_DESC_FRIEND;
import static seedu.address.logic.commands.CommandTestUtil.SUBJECT_DESC_HUSBAND;
import static seedu.address.logic.commands.CommandTestUtil.VALID_ADDRESS_AMY;
import static seedu.address.logic.commands.CommandTestUtil.VALID_EMAIL_AMY;
import static seedu.address.logic.commands.CommandTestUtil.VALID_NAME_AMY;
import static seedu.address.logic.commands.CommandTestUtil.VALID_PHONE_AMY;
import static seedu.address.logic.commands.CommandTestUtil.VALID_PHONE_BOB;
import static seedu.address.logic.commands.CommandTestUtil.VALID_SUBJECT_FRIEND;
...
public class EditCommandParserTest {
    ...
    public void parse_invalidValue_failure() {
        ...
        // while parsing {@code PREFIX_SUBJECT} alone will reset the subjects of the {@code Student} being edited,
        // parsing it together with a valid subject results in error
        assertParseFailure(parser, "1" + SUBJECT_DESC_FRIEND + SUBJECT_DESC_HUSBAND
                + SUBJECT_EMPTY, Subject.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, "1" + SUBJECT_DESC_FRIEND + SUBJECT_EMPTY
                + SUBJECT_DESC_HUSBAND, Subject.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, "1" + SUBJECT_EMPTY + SUBJECT_DESC_FRIEND
                + SUBJECT_DESC_HUSBAND, Subject.MESSAGE_CONSTRAINTS);

        // multiple invalid values, but only the first invalid assignmentName is captured
    ...
    public void parse_allFieldsSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_BOB + SUBJECT_DESC_HUSBAND
                + EMAIL_DESC_AMY + ADDRESS_DESC_AMY + NAME_DESC_AMY + SUBJECT_DESC_FRIEND;
    ...
    public void parse_oneFieldSpecified_success() {
        ...
        descriptor = new EditStudentDescriptorBuilder().withSubjects(VALID_SUBJECT_FRIEND).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
    }
    ...
    public void parse_multipleRepeatedFields_failure() {
        ...
        // AddStudentCommandParserTest#parse_repeatedNonSubjectValue_failure()
        ...
        userInput = targetIndex.getOneBased() + PHONE_DESC_AMY + ADDRESS_DESC_AMY + EMAIL_DESC_AMY
                + SUBJECT_DESC_FRIEND + PHONE_DESC_AMY + ADDRESS_DESC_AMY + EMAIL_DESC_AMY + SUBJECT_DESC_FRIEND
                + PHONE_DESC_BOB + ADDRESS_DESC_BOB + EMAIL_DESC_BOB + SUBJECT_DESC_HUSBAND;
